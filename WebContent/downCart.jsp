...
<body>
	<%
		...
		for(int i = 0; i< list.size(); i++){
			goodsQnt = list.get(i);
			if(goodsQnt.getProductId().equals(id)){ //동일한 상품을 선택하면 해당 상품의 수량을 감소시킴
				cnt++;			
				int orderQuantity = goodsQnt.getQuantity() - 1;
				goodsQnt.setQuantity(orderQuantity); //화면에도 1을 감소시킴
				
				if(orderQuantity == -1){
					for(i=0;i<list.size();i++){
						goodsQnt = list.get(i);
						if(goodsQnt.getProductId().equals(id)){ //지우고 싶은 아이디 값을 받아옴
							list.remove(goodsQnt); //remove : arrayList에 내용을 지움
						}
					}						
				}
			}	
